...
<body>
	...
	<form name="form1" method="post" action="/board/list.do">
		...
		<select name="search_option">
			...
			<option value="all"
				<c:out value="${map.search_option == 'all' ? 'selected' : ''}"/>>
				이름+내용+제목</option>

			<option value="name"
				<c:out value="${map.search_option == 'name' ? 'selected' : ''}"/>>
				이름</option>

			<option value="contents"
				<c:out value="${map.search_option == 'contents' ? 'selected' : ''}"/>>
				내용</option>

			<option value="title"
				<c:out value="${map.search_option == 'title' ? 'selected' : ''}"/>>
				제목</option>
	...
	<table border="1" width="600px">
		...
		<c:forEach var="row" items="${map.list}">
			<tr align="center">
				...
				<td>
				<a 	href="/board/detail.do?idx=${row.idx}
					&cur_page=${map.page_info.curPage}
		...
		<tr>
			<td colspan="5" align="center">
				...
				<c:forEach var="num" 
							begin="${map.page_info.blockBegin}"
